...
## 인터랙션 (v3)
- **구역 = 다중 토글**(`enabledZones` Set, 기본 전부 ON, localStorage `"ds_zones"` 지속): 켠 구역만 표시. `buildZoneTabs`가 구역 버튼+"전체 ON" 렌더, 최소 1개 강제(마지막 해제 차단). 셀 클릭 → **(라인+층) 통합 편집 모달**(횡주·입상·바닥 한 번에, 모바일 하단시트).
...
- 메모 3종(층 `fm:all:<층>`(구역 다중토글 이후 키 전역화), 라인 `lm:`, 잔여작업 `rm:<라인>:<층>`) + 접점(`uc:`/`dc:`). 사진은 `ph:<라인>:<층>`(Supabase Storage 버킷 `photos`, 로컬모드는 base64). 층 hover 시 해당 층 강조.
- **변경 이력**(`📜`, 실시간 모드 전용): `change_log`에서 누가(이름·공구)·언제·이전→이후. **일자 select + 일자별 그룹 헤더**(`histDateSel`).
- 범례·갯수 바: **모바일은 기본 접힘**(저장값 없을 때 `matchMedia(max-width:760px)`), 토글로 펴기. 해당없음 칩은 갯수 숫자 생략.

## 로그인 (읽기 누구나 · 수정만)
- **읽기는 누구나, 수정만 로그인**. 시작 시 로그인 모달(닫기 가능 → 읽기 전용). `openEditor`/`openText`가 미로그인 시 `openLogin()` 게이트.
- `USER = {name, team}` 의 `team` 에 **공구**(`"1공구"`…`"5공구"`) 저장(localStorage `"ds_user"`, 자동 로그아웃 없음). `change_log.team`·`userLabel`·동시편집 토스트가 그대로 동작.
- 비번 = `GONGGU_PW` 상수(공구별), **클라이언트 측 화면 잠금**일 뿐(익명 쓰기 허용 → 진짜 차단은 Supabase Auth 필요). 공구는 기록용(공구5·구역4라 1:1 매핑 없음).
- **공유 명단(roster)**: notes 테이블 `"roster:<공구>"` = JSON 이름 배열. 새 이름 로그인 시 append → `store.setNote`. view-only에서도 읽힘(boot에서 notes 로드).
