...
- **대각선(╱, 회색 얇게) = 횡주간 없음**(횡주 레이어 전용). **입상이 있어야 횡주가 있음** → 입상 있고 횡주 없으면 **입상 색을 따라가고 대각선 하나** 표시(`riserColor()`). 입상 없는 층의 횡주 '해당없음'은 대각선 없이 흰색. 편집기에서 '횡주간 없음' 칩으로 직접 지정·복원 가능(no_beam).
